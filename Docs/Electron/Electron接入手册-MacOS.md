...
> 📖 本文档将指导您如何在 Electron 项目中快速接入腾讯会议 SDK
...
${SDK_ROOT}/
├── SDK/                          # SDK 核心文件
│   ├── TMSDK.framework           # 通用二进制 Framework（x86_64 + arm64）
│   └── mac_split_framework       # SDK 架构拆分脚本
│
...
### 步骤 2：拷贝 SDK Framework 文件
将对应架构的 `TMSDK.framework` 拷贝到项目的 `Frameworks` 目录：

```bash
# 创建 Frameworks 目录
mkdir -p Frameworks

# 拷贝对应架构的 Framework（以 arm64 为例）
...
在 package.json 中配置启动脚本，需要将 Frameworks 复制到 Electron 应用目录：
...
> ⚠️ `rsync` 命令用于在启动时将 SDK Frameworks 复制到 Electron 应用目录，确保运行时能找到动态库

### 步骤 5：配置主进程加载 SDK

创建 main.js，在主进程中加载 SDK 并设置 IPC 通信：
...
- >= 3.12.1版本：需要传 `app_icon` 参数，Mac端没有app_icon定制场景建议传默认值 `""` 空串
